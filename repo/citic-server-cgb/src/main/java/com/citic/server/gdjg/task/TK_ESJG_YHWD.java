 ...
 * 
 * @author liuxuanfei
 * @date 2017年6月2日 下午3:30:49
 */
public class TK_ESJG_YHWD extends NBaseTask{
	private static final Logger logger = (Logger) LoggerFactory.getLogger(TK_ESJG_YHWD.class);
	private RequestMessageServiceGdjg requestSend;
	private MM57_cxqqMapper mm57_cxqqMapper;

	public TK_ESJG_YHWD(ApplicationContext ac, MC21_task_fact mC21_task_fact) {
	...
	public boolean calTask() throws Exception {
		...
		try {
				...
				//2. 获取银行网点登记表(需上报)
				Gdjg_RequestWddj wddj = new Gdjg_RequestWddj();
				List<Gdjg_RequestWddj_Branch> branchs = mm57_cxqqMapper.getBr40_branch_regList(mc21_task_fact);
				wddj.setBranchs(branchs);
				if(branchs.size()==0){
					return false;
				}
				//调用上报接口
				Gdjg_Response response = requestSend.sendYhwddjMassage(wddj);
			
				fkBo.insertBr57_receipt(response,"YHWD");
