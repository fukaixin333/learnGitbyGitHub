 ...
 */
@Service("remoteDataOperateCBRC")
public class RemoteDataOperateCBRC implements IDataOperateCBRC {
	...
	public CBRC_QueryRequest_Customer getAccountDetailAndTransaction(Br40_cxqq_mx cxqq) throws DataOperateException, RemoteAccessException {
		CBRC_QueryRequest_Customer customer = null; // 客户信息
		...
		if (ServerEnvironment.TASK_TYPE_SHENZHEN.equals(taskType)) {
			// 深圳公安局直接获取查询方式
			queryType = cxqq.getCxfs();
		} else {
			// 公安部经侦、国安根据“查询类型”进行判断，该值并非监管给，而是Task根据查询账号是否为空赋予
		...
		List<CBRC_QueryRequest_Account> accountList = new ArrayList<CBRC_QueryRequest_Account>();
		// 根据账（卡）号查询
		...
		List<CBRC_QueryRequest_Measure> measureList = new ArrayList<CBRC_QueryRequest_Measure>();
		List<CBRC_QueryRequest_SubAccount> subAccountList = new ArrayList<CBRC_QueryRequest_SubAccount>();
		List<CBRC_QueryRequest_Transaction> transactionList = new ArrayList<CBRC_QueryRequest_Transaction>();
		List<CBRC_QueryRequest_Priority> prioritiesList = new ArrayList<CBRC_QueryRequest_Priority>();
		for (String accountNumber : accountNumbers) {
			// 查询合约账户的共有权/优先权信息 
			OwnershipInfo joInfo = messageService.queryOwnershipInfo(accountNumber);
			AccountDetail accountDetail = messageService.queryAccountDetail(accountNumber, null, null);
			...
			if ("01".equals(subjectType)) {
				...
				if (subAccountInfos == null || subAccountInfos.size() == 0) {
					...
					for (SubAccountInfo subAccount : subAccountInfos) {
						...
						OwnershipInfo subJoInfo = messageService.queryOwnershipInfo(subAccountNumber);
						...
						if (subJoInfo != null && (subJoInfo.isMargin() || subJoInfo.isPledge())) {
							prioritiesList.add(copyQueryRequestQlxx(accountNumber, subAccountDetail));
						}
					}
				}
			} else {
				if (joInfo != null && (joInfo.isMargin() || joInfo.isPledge())) {
					prioritiesList.add(copyQueryRequestQlxx(accountNumber, accountDetail));
				}
			}
	...
	public CBRC_QueryRequest_Priority copyQueryRequestQlxx(String accountNumber, AccountDetail accountDetail) {
		CBRC_QueryRequest_Priority qlxx = new CBRC_QueryRequest_Priority();
		qlxx.setZh(accountNumber);// 账号
		qlxx.setQllx("优先受偿权类型"); // 权利类型
		qlxx.setQlrxm("广发银行股份有限公司"); // 权利人
		
		// 若账户[可用余额]为空，则将[账面余额]视为[权利金额]
		if (accountDetail.getAvailableBalance() == null || accountDetail.getAvailableBalance().length() == 0) {
			qlxx.setQlje(accountDetail.getAccountBalance()); // 权利金额/数额
		} else {
			qlxx.setQlje(accountDetail.getAvailableBalance()); // 权利金额/数额
		}
	...
	public List<CBRC_QueryRequest_Transaction> getAccountTransaction(Br40_cxqq_mx cxqq) throws DataOperateException, RemoteAccessException {
		...
		// 根据证件类型、证件号码、主体名称信息查询
			...
			// 查询客户信息
			String subjectType = cxqq.getZtlb(); // 主体类别
			CBRC_QueryRequest_Customer customer = queryCustomerByCredentialInfo(subjectType, credentialType, credentialNumber, subjectName);
			if (customer == null) {
				throw new DataOperateException(CBRCConstants.REC_CODE_99999, "查无客户信息");
			}
	...
	public CBRC_FreezeRequest_Record freezeAccountJC(Br41_kzqq kzqq, CBRC_FreezeResponse_Account res) throws DataOperateException, RemoteAccessException {
		...
		CBRC_FreezeRequest_Record unfreezeRecord = new CBRC_FreezeRequest_Record();
	...
	public CBRC_StopPaymentRequest_Recored stoppayAccount(Br41_kzqq kzqq, CBRC_StopPaymentResponse_Account res) throws DataOperateException, RemoteAccessException {
		...
		CBRC_StopPaymentRequest_Recored stopPayment = new CBRC_StopPaymentRequest_Recored();
	...
	public CBRC_StopPaymentRequest_Recored stoppayAccountJC(Br41_kzqq kzqq, CBRC_StopPaymentResponse_Account res) throws DataOperateException, RemoteAccessException {
		...
		CBRC_StopPaymentRequest_Recored restore = new CBRC_StopPaymentRequest_Recored();
	...
	protected CBRC_QueryRequest_Customer queryCustomerByAccountNumber(String subjectType, String primaryNumber) throws DataOperateException, RemoteAccessException {
		if ("01".equals(subjectType)) {
			// 个人客户信息 
	...
	protected CBRC_QueryRequest_Customer queryCustomerByCredentialInfo(String subjectType, String credentialType, String credentialNumber, String subjectName)
	...
	//                     对象转换的接口函数
	// ==========================================================================================
	private CBRC_FreezeRequest_Record copyFreezeResultProperties(FreezeResult freezeResult) {
		if (freezeResult == null) {
			return null;
		}
		
		CBRC_FreezeRequest_Record freezeRecord = new CBRC_FreezeRequest_Record();
	...
	private CBRC_FreezeRequest_Record copyFreezeResultProperties(DeferFreezeResult freezeResult) {
		if (freezeResult == null) {
			return null;
		}
		
		CBRC_FreezeRequest_Record deferFreezeRecord = new CBRC_FreezeRequest_Record();
	...
	private CBRC_QueryRequest_Customer copyPersonalInfoProperties(IndividualCustomer personalInfo) {
		if (personalInfo == null) {
			return null;
		}
		
		CBRC_QueryRequest_Customer customer = new CBRC_QueryRequest_Customer();
	...
	private CBRC_QueryRequest_Customer copyCorporateInfoProperties(CorporateCustomer corporateInfo) {
	...
	private CBRC_QueryRequest_Account copyAccountProperties(AccountDetail accountDetail) {
	...
	private List<CBRC_QueryRequest_Transaction> copyTransactionProperties(String cardNumber, V_AccountTransaction v) {
		return copyTransactionProperties(cardNumber, v.getAccountTransactionList(), v.getAccountName());
	}
	
	private List<CBRC_QueryRequest_Transaction> copyTransactionProperties(String cardNumebr, List<AccountTransaction> list, String accountName) {
		List<CBRC_QueryRequest_Transaction> transactionList = new ArrayList<CBRC_QueryRequest_Transaction>();
		for (AccountTransaction transaction : list) {
			...
			CBRC_QueryRequest_Transaction cbrc_query_transaction = new CBRC_QueryRequest_Transaction();
	...
	private List<CBRC_QueryRequest_Measure> copyFreezeMeasureProperties(List<AccountFrozenMeasure> measures) {
		List<CBRC_QueryRequest_Measure> measureList = new ArrayList<CBRC_QueryRequest_Measure>();
		for (AccountFrozenMeasure measure : measures) {
			if ("N".equals(measure.getFrozenStatus())) { // N-冻结类数据
				CBRC_QueryRequest_Measure cbrc_query_measure = new CBRC_QueryRequest_Measure();
	...
	private CBRC_QueryRequest_SubAccount copySubAccountProperties(AccountDetail detail, String cardNumber, String serial) {
		CBRC_QueryRequest_SubAccount subAccount = new CBRC_QueryRequest_SubAccount();
