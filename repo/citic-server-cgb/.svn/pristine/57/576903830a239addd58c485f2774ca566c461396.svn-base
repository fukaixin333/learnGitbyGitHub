...
public abstract class BaseFile extends Base {
	...
	public boolean initDataFileForLoad() throws Exception {
		...
		/**
		 * 1、如果是客户送数据到本地，数据先进入LDSPATH，计算时改目录名字，放入LWORKPATH下带日期的子目录内（YYYY-MM-
		 * DD_DSID）
		...
		String ldatapathroot = lworkpath + datatime + "_" + this.getMC00_datasource().getDsid();

		String dgetmethod = this.getMC00_datasource().getDgetmethod();
		String dputlocation = this.getMC00_datasource().getDputlocation();

		if (dgetmethod.equalsIgnoreCase("put") && dputlocation.equalsIgnoreCase("localfile")) {// 客户送到本地文件
		...
		//	ldatapathroot = ldspath;
			if (!ldspath.equalsIgnoreCase(ldatapathroot)) {// 可能重名

				/*if (!FileUtils.renameFile(ldspath, ldatapathroot)) {
					logger.error("文件重命名失败：源[" + ldspath + "] 目标[" + ldatapathroot + "]!!!");
					return isSucc;
				}*/
		...
		strHash.put(this.getMC00_datasource().getLworkpath(), lworkpath);
		...
		/**
		 * 当数据库装在脚本中的数据库连接信息是通过：tool-config.xml统一配置时，如下生效 如果密码被Base64编码，此处需要解开
		 */
		...
		String back_dataload_database_dbname = applicationProperties.getBackDataLoadDatabase_dbname();
		String back_dataload_database_username = applicationProperties.getBackDataLoadDatabase_username();
		String back_dataload_database_password = applicationProperties.getBackDataLoadDatabase_password();
		String back_dataload_flag_dbname = applicationProperties.getBackDataLoadFlag_dbname();
		String back_dataload_flag_username = applicationProperties.getBackDataLoadFlag_username();
		String back_dataload_flag_password = applicationProperties.getBackDataLoadFlag_password();
		if (back_dataload_database_dbname != null && !back_dataload_database_dbname.equals("")
			...
			// //数据库密码解密
			// back_dataload_database_password =
			// Base64Utils.decodeBase64(back_dataload_database_password);
			// }
			// if(back_dataload_database_dbname.lastIndexOf("==")>-1){
			// //数据库密码解密
			// back_dataload_database_dbname =
			// Base64Utils.decodeBase64(back_dataload_database_dbname);
			// }
	...
	int backdays = applicationProperties.getDatafileBackupDays();
		//int backdays = 2;
		if (backdays <= 0)
		...
		//清理数据文件
		File file_data = new File(lworkpath +sdf1.format(currDatatime.toDate() ) );
		while (true) {
			if (!file_data.exists()) {
				break;
			}
			logger.info("清理历史数据:" + file_data.getName());
			// 清理当前目录

			deleteHistoryDate(file_data);

		}				
		//清理跑批后生成的目录文件 
		SimpleDateFormat sdf2=new SimpleDateFormat("yyyy-MM-dd");
				File file_Ds = new File(lworkpath +sdf2.format(currDatatime.toDate() ) +"_"+dsid);
				while (true) {
					if (!file_Ds.exists()) {
						break;
					}
					logger.info("清理历史数据:" + file_Ds.getName());
					// 清理当前目录
					deleteHistoryDate(file_Ds);
				}
	}
