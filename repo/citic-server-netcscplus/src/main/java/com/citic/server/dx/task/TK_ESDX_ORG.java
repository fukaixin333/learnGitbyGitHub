...
public class TK_ESDX_ORG extends NBaseTask {

	private static final Logger logger = (Logger) LoggerFactory.getLogger(TK_ESDX_ORG.class);
	...
	public TK_ESDX_ORG(ApplicationContext ac, MC21_task_fact mC21_task_fact) {
	...
	public boolean calTask() throws Exception {
		...
		try {
			...
			String oigTaskkey=mc21_task_fact.getTgroupid();  //1高法 2电信诈骗
  ...
  if(!cardnumber.equals("")){
	  ...
	  msgCheckResult="2";  
	   result="0500";
			...
			this.updateOrgkeyStr(sqlStr, organkey, regtype,msgCheckResult,result);			
			...
			String isemploye=mc21_task_fact.getIsdyna();
			 MC21_task_fact  mc21_task_fact1=new  MC21_task_fact();
			    ...
			    if(!msgCheckResult.equals("1")){ //走人工单非本行客户直接生成task3	\
			    	mc21_task_fact1.setIsemployee("0");
					  kzqqBo.insertMc21TaskFact3(mc21_task_fact1, "2");
				  }else{

					  if(isemploye.equals("0")){
					  kzqqBo.insertMc21TaskFact2Fo(mc21_task_fact1, "2");
					  }
				  }
			  if(isemploye.equals("1")){
				    if(msgCheckResult.equals("1")){ //走人工单本行客户发短信
						  kzqqBo.sendMsg(organkey, mc21_task_fact1.getTaskname(),mc21_task_fact1.getSubtaskid(),type,regtype,"1");
					  }
			  }
...
public void updateOrgkeyStr(String upSqlStr, String orgkey, String reptype,String msgCheckResult,String result) throws Exception {
  ...
  if(msgCheckResult.equals("1")){
		...
		if (objs.length > 1) {
			...
			MP02_rep_org_map mp02_rep_org_map = (MP02_rep_org_map) reportOrganMap.get(reptype + orgkey);
			if (mp02_rep_org_map != null) {
				String operatorname = mp02_rep_org_map.getOperatorname();
				String operatorphonenumber = mp02_rep_org_map.getOperatorphonenumber();
				String sqlStr = objs[1];
				if (sqlStr != null && !sqlStr.equals("")) {
					String[] objstrs = sqlStr.split(";");
					String objstr1 = objstrs[0].replaceAll("@B@", "'" + operatorname + "'");
					objstr1 = objstr1.replaceAll("@C@", "'" + operatorphonenumber + "'");
					jdbcTemplate.update(objstr1);
					if (objstrs.length > 1) {
						String objstr2 = objstrs[1].replaceAll("@C@", "'" + operatorphonenumber + "'");
						objstr2 = objstr2.replaceAll("@B@", "'" + operatorname + "'");
		...
		if (objs.length > 2) {//修改相应表中的验证标识
			String sqlStr = objs[2];
			String[] objstrs = sqlStr.split(";");
			String objstr1 = objstrs[0].replaceAll("@D@", "'" + msgCheckResult + "'");
			objstr1 = objstr1.replaceAll("@E@", "'" + result + "'");
			jdbcTemplate.update(objstr1);
			if (objstrs.length > 1) {
