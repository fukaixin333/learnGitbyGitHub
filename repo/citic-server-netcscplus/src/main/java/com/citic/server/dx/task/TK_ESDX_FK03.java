 ...
 * @version 1.0
 */
public class TK_ESDX_FK03 extends NBaseTask {
	
	private static final Logger logger = (Logger) LoggerFactory.getLogger(TK_ESDX_FK03.class);
	
	public TK_ESDX_FK03(ApplicationContext ac, MC21_task_fact mC21_task_fact) {
	...
	public boolean calTask() throws Exception {
		...
		try {
			...
			//1.获取账户动态查询反馈DTORequest100306
			request = q_main_backBo.setQueryRequest100306(mc21_task_fact);
			
			//2.调用接口发送
			Response response = requestMessageService.sendQueryRequest100306(request);
			
			//3.插入响应表
			response.setTransSerialNumber(mc21_task_fact.getTgroupid()); //对应baseinfo表的流水号
			...
			br24_bas_info.setTransSerialNumber(mc21_task_fact.getTgroupid());
