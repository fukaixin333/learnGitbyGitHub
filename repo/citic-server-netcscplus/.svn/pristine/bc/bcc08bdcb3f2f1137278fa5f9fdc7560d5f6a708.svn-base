...
public class TK_ESDX_FK02 extends NBaseTask {
	
	private static final Logger logger = (Logger) LoggerFactory.getLogger(TK_ESDX_FK02.class);
	
	public TK_ESDX_FK02(ApplicationContext ac, MC21_task_fact mC21_task_fact) {
	...
	public boolean calTask() throws Exception {
		...
		try {
			...
			//1.获取账户持卡主体查询反馈DTORequest100304
			request = q_main_backBo.setQueryRequest100304(mc21_task_fact);
			
			//2.调用接口发送
			Response response = requestMessageService.sendQueryRequest100304(request);
			//3.插入响应表
