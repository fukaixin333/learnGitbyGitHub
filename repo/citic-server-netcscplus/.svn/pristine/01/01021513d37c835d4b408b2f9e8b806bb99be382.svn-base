...
/**
 * 黑/灰名单轮序（获取）任务 1.获取远程ftp目录下的所有文件列表 2、与本地已处理完成的文件列表，进行比较，找出待处理文件 2.1获取最后一个成功加载的quan l 3、循环处理文件
 * 3.0.1 全量／增量文件，检查该文件是否已加载 3.0.2 增量文件： 3.1通过ftp，下载指定文件 3.2 解析指定文件，插入名单数据表 3.3
 * 处理完毕，写文件处理成功标志，以便于下次比较
 * 
 * @author Liu Xuanfei
 * @date 2016年4月18日 上午11:29:23
 */
@Component("suspiciousOuterPollingTask")
public class SuspiciousOuterPollingTask extends AbstractPollingTask {
	...
	@Autowired
	private Br20_md_infoMapper br20_md_infoMapper;
	...
	public void executeAction() {
		try {
			...
			if (StringUtils.equals("", max_all_black_file_name)) {// 最近一个黑名单全量文件
				...
				String all_black_name = StrUtils.null2String(br20_md_infoMapper.getMaxFileName(md_info));
			...
			if (StringUtils.equals("", max_all_gray_file_name)) {// 最近一个灰名单全量文件
				...
				md_info.setFile_type("2");// 1增量／2全量
				String all_gray_name = StrUtils.null2String(br20_md_infoMapper.getMaxFileName(md_info));
			...
			if (StringUtils.equals("", max_black_file_name)) {// 最近一个黑名单增量文件
				...
				String black_name = StrUtils.null2String(br20_md_infoMapper.getMaxFileName(md_info));
			...
			if (StringUtils.equals("", max_gray_file_name)) {// 最近一个灰名单增量文件
				...
				md_info.setMd_type("2");// 1黑／2灰
				md_info.setFile_type("1");// 1增量／2全量
				String gray_name = StrUtils.null2String(br20_md_infoMapper.getMaxFileName(md_info));
	...
	private void dealAllBlackFile(FtpUtils ftp, String remotepath, String localpath, Set<String> set, String alldbafilename) throws Exception {
		...
		String flag = br20_md_infoMapper.getAllLoadFlag();// 0：全量加载关闭 1:全量加载开启
		if (StringUtils.equals("1", flag)) {
			...
			if (!StringUtils.equals("", dealname)) {
				...
				br20_md_infoMapper.insertBr20_md_accept(filelog);// 插入一条文件接收日志
	...
	private void dealIncrementBlackFile(FtpUtils ftp, String remotepath, String localpath, Set<String> set, String dbafilename, String alldbafilename) throws Exception {
		...
		if (deallist != null && deallist.size() > 0) {
			...
			logger.info("==本次增量黑名单轮询结果:下载文件数：" + deallist.size() + "==处理成功文件数：" + m + "==解析异常文件数：" + n);
		} else {
	...
	private void dealIncrementGrayFile(FtpUtils ftp, String remotepath, String localpath, Set<String> set, String dbafilename, String alldbafilename) throws Exception {
		...
		if (deallist != null && deallist.size() > 0) {
			...
			logger.info("==本次增量灰名单轮询结果:下载文件数：" + deallist.size() + "==处理成功文件数：" + m + "==解析异常文件数：" + n);
		} else {
	...
	/**
	 * 获取库存待处理文件 包括已接受的，处理失败的状态的。
	 * 
	 * @param md_type
	 * @param file_type
	 * @return
	 */
	//	private List<String> getToDealFileNameList(String md_type, String file_type) {
	//		Br20_md_info md_info = new Br20_md_info();
	//		md_info.setFlag("'1','2'");
	//		md_info.setMd_type(md_type);// 1黑／2灰
	//		md_info.setFile_type(file_type);// 1增量／2全量
	//		// md_info.setAccept_time(DtUtils.getNowDate());//取当天日期，查询时可缩小范围
	//		List<String> to_filenamelist = Lists.newArrayList();
	//		to_filenamelist = br20_md_infoMapper.getDealFileName(md_info);
	//
	//		return to_filenamelist;
	//	}
	...
	private Br20_md_info getFileData(String localpath, String filename, String md_type, String file_type) {
		...
		try {
			...
			try {
				...
				br20_md_infoMapper.deleteBr20_md_dataByFlagAndBatch(file_code);
				//插入最新的有效记录
				br20_md_infoMapper.insertBr20_md_dataByNewAndBatch(filelog);
			} catch (Exception e) {
	...
	private Set<String> getWhiteListSet() {
		Set<String> s = null;
		List<Br20_md_info> whitelist = br20_md_infoMapper.getWhiteList("");
	...
	private Br20_md_info getFileDataAll(String localpath, String filename, String md_type, String file_type, Set<String> set) {
		...
		try {
			...
			// 清理数据日志表和数据表中的数据
			br20_md_infoMapper.deleteBr20_md_data_log(md_type);
			br20_md_infoMapper.deleteBr20_md_dataByVo(md_type);
						...
						if (whiteflag && set.contains(key)) {// 数据存在于白名单中，不入库
							...
							if (!StringUtils.equals("", str[9])) {// 公安审核通过黑名单时间戳 19
								...
								if (str[9].length() == 14) {
									...
									logger.warn("====公安审核通过黑名单时间戳:" + str[9] + "===约定长度14，长度非法,将取默认值空字符串");
								}
							}
							if (!StringUtils.equals("", str[10])) {// 名单类型冗余项(原先的开卡时间)
								// 解决人工造数长度非法的问题
			...
			if (r == null && list != null && list.size() > 0) {// 文件最后一次读区，不满足BATCH_NUM的条数
				doBatchSql(list);// 批量处理
				list.clear();
			}
			filelog.setFlag("3");
		} catch (Exception e) {
			logger.error("==全量文件处理异常，请检查数据内容合理性==文件名称：" + filename + "==当前读取有效数据行" + count);
	 ...
	 * @param list
	 */
	private void doBatchSql(List<Br20_md_info> list) {
		...
		for (Br20_md_info md : list) {
			Object[] values_log = new Object[] {md.getFile_code(), md.getMd_code(), md.getMd_type(), md.getMd_kind(), md.getCardtype(), md.getMd_value(), md.getP_name(),
												md.getMd_source(), md.getCase_type(), md.getOperate_flag(), md.getSend_flag(), md.getAccept_time(), md.getBank_id(),
												md.getPolice(), md.getPolice_phone(), md.getPolice_check_dt(), md.getOpen_card_dt(), md.getStart_dt(), md.getEnd_dt(),
												md.getRemark()};
			Object[] values_data = new Object[] {md.getMd_code(), md.getMd_type(), md.getMd_kind(), md.getCardtype(), md.getMd_value(), md.getP_name(), md.getMd_source(),
													md.getCase_type(), md.getMd_flag(), md.getLast_update_time(), md.getLast_update_user(), md.getBank_id(), md.getPolice(),
													md.getPolice_phone(), md.getPolice_check_dt(), md.getOpen_card_dt(), md.getStart_dt(), md.getEnd_dt(), md.getRemark()};
			batch_log.add(values_log);
			batch_data.add(values_data);
		}
		String sql_log = "insert into br20_md_data_log(file_code,md_code,md_type,md_kind,cardtype,md_value,p_name,md_source,case_type,operate_flag,send_flag,accept_time,bank_id,police,police_phone,police_check_dt,open_card_dt,start_dt,end_dt,remark)"
							+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	/**
	 * 文件是否处理 文件名格式必须一致，比较才会准确 filename比dbname，说明文件没有处理过
	 * 
	 * @param filename
	 *        待处理文件
	 * @param dbname
	...
	/**
	 * 轮询名单时间间隔 cfca每5分钟左右生成一个文件，此处设置为5分钟
	 */
	@Override
