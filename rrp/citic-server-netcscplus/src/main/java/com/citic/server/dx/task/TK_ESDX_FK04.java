...
public class TK_ESDX_FK04 extends NBaseTask {
	
	private static final Logger logger = (Logger) LoggerFactory.getLogger(TK_ESDX_FK04.class);
	
	public TK_ESDX_FK04(ApplicationContext ac, MC21_task_fact mC21_task_fact) {
	...
	public boolean calTask() throws Exception {
		...
		try {
			...
			//1.获取账户动态查询解除反馈DTORequest100308
			request = q_main_backBo.setQueryRequest100308(mc21_task_fact);
			
			//2.调用接口发送
			Response response = requestMessageService.sendQueryRequest100308(request);
