...
public class TK_ESDX_JS02 extends NBaseTask {

	private static final Logger logger = (Logger) LoggerFactory.getLogger(TK_ESDX_JS02.class);
	private CacheService cacheService;

	public TK_ESDX_JS02(ApplicationContext ac, MC21_task_fact mC21_task_fact) {
	...
	public boolean calTask() throws Exception {
		...
		try {
			...
			QueryResponse100303 query = (QueryResponse100303) CommonUtils.unmarshallUTF8Document(QueryResponse100303.class, flie_path);

			//删除请求单号下的数据
			q_mainBo.delQureyResponse(mc21_task_fact.getBdhm());

			// 插入查询请求单主表
			q_mainBo.insertBr24_q_main100303(query,orgkey,mc21_task_fact.getIsemployee());
