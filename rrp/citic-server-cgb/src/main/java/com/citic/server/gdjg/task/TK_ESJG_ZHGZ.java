 ...
 * 
 * @author liuxuanfei
 * @date 2017年6月2日 下午3:31:14
 */
public class TK_ESJG_ZHGZ extends NBaseTask{
	private static final Logger logger = (Logger) LoggerFactory.getLogger(TK_ESJG_ZHGZ.class);
	private RequestMessageServiceGdjg requestSend;
	MM57_cxqqMapper mm57_cxqqMapper;
	
	
	public TK_ESJG_ZHGZ(ApplicationContext ac, MC21_task_fact mC21_task_fact) {
	...
	public boolean calTask() throws Exception {
		...
		try {
				...
				//2. 获取银行网点登记表(需上报)
				Gdjg_RequestGzdj gzdj = new Gdjg_RequestGzdj();
				List<Gdjg_RequestGzdj_AccountRule> accountrules = mm57_cxqqMapper.getBr40_acct_ruleList(mc21_task_fact);
				gzdj.setAccountrules(accountrules);
				if(accountrules.size()==0){
					return false;
				}
				//调用上报接口
				Gdjg_Response response = requestSend.sendYhzhgzdjMassage(gzdj);
			
				fkBo.insertBr57_receipt(response,"ZHGZ");
				//3.插入基础数据发送日志表
