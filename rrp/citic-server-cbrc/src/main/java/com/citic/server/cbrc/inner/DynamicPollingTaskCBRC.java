package com.citic.server.cbrc.inner;
...
import com.citic.server.cbrc.mapper.CBRC_OuterPollingTaskMapper;
 ...
 */
@Service("dynamicPollingTaskCBRC")
public class DynamicPollingTaskCBRC extends AbstractPollingTask {
	private final Logger logger = LoggerFactory.getLogger(DynamicPollingTaskCBRC.class);
	
	@Autowired
	private MM40_cxqq_cbrcMapper mm40_cxqqMapper;
	
	private BaseBo baseBo;
	private IDataOperateCBRC remoteDataOperate;
	...
	@Autowired
	private CBRC_OuterPollingTaskMapper cbrc_mapper;
	...
	public void executeAction() {
		...
		// 关闭截止时间到期的任务
		cbrc_mapper.closeBr40_cxqq_acct_dynamic_main(currDataTime);
		
		// 将动态查询到期的数据的状态改为3
		String currdate = Utility.currDate10();
		cbrc_mapper.updateBr41_cxqq_back_dynamic(currdate);
		
		// 获取所有待处理任务
		List<Br40_cxqq> dynamicsTasks = cbrc_mapper.queryBr40_cxqq_acct_dynamic_main(null, currDataTime);
	...
	private void processDynamicTask(Br40_cxqq dynamics) throws Exception {
		...
		if (transactionList == null || transactionList.size() == 0) {
			...
			List<CBRC_QueryRequest_Transaction> newTransList = new ArrayList<CBRC_QueryRequest_Transaction>(size);
		...
		// 更新本次处理时间
		cbrc_mapper.updateBr40_cxqq_acct_dynamic_main(taskType, rwlsh, currDataTime, String.valueOf(nextSeq));
	}
	...
	public void insertTask3(Br40_cxqq_mx br40_cxqq_mx, String seq, Br40_cxqq _cxqq) throws Exception {
		...
		getBaseBo().insertMc21TaskFact3(taskFact, "CBRC_" + _cxqq.getTasktype());
	}
	 ...
	 * @return
	 */
	private IDataOperateCBRC getRemoteDataOperate() {
		if (remoteDataOperate == null) {
			remoteDataOperate = SpringContextHolder.getBean("remoteDataOperateCBRC");
		}
	...
	protected String getExecutePeriodExpression() {
		return ServerEnvironment.getStringValue(CBRCKeys.INNER_POLLING_PERIOD_DYNAMICS, "每天|02:00");
	}
}
