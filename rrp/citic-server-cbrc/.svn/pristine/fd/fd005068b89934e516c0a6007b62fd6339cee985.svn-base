...
import com.citic.server.cbrc.domain.response.CBRC_ControlResponse_Account;
import com.citic.server.cbrc.domain.response.CBRC_QueryResponse;
import com.citic.server.cbrc.domain.response.CBRC_QueryResponse_Main;
import com.citic.server.cbrc.domain.response.CBRC_QueryScanResponse;
import com.citic.server.cbrc.domain.response.CBRC_QueryScanResponse_Info;
...
public class Cxqq_JSBo extends BaseBo {
	...
	public Cxqq_JSBo(ApplicationContext ac) {
		super(ac);
		br40_cxqqMapper = (MM40_cxqq_cbrcMapper) ac.getBean("MM40_cxqq_cbrcMapper");
	 ...
	 * @throws Exception
	 */
	public void handleCxqqBw(CBRC_QueryResponse query_cgcx, MC21_task_fact mc21_task_fact, MC20_task_msg taskmag) throws Exception {
		CBRC_BasicInfo basicinfo = query_cgcx.getBasicInfo();
		...
		//2.插入请求主体信息、反馈主体信息
		List<CBRC_QueryResponse_Main> querymain_cgList = query_cgcx.getQueryMainList();
		for (CBRC_QueryResponse_Main querymain_Cg : querymain_cgList) {
			...
			this.insertMc21TaskFact2(mc21_task_fact, "CBRC_" + tasktype);
		}
	 ...
	 * @throws Exception
	 */
	public void insertBr40_cxqq_dt(CBRC_ControlResponse query, MC21_task_fact mc21_task_fact, MC20_task_msg taskmag) throws Exception {
		...
		//2. 插入请求主体信息、反馈主体信息
		List<CBRC_ControlResponse_Account> dtList = query.getControlAccountList();
		for (CBRC_ControlResponse_Account querymain_dt : dtList) {
			...
			//2.3 插入task2
			mc21_task_fact.setBdhm(qqdbs + "$" + rwlsh);
			this.insertMc21TaskFact2(mc21_task_fact, "CBRC");
		}
	}
	 ...
	 * @throws Exception
	 */
	public void insertBr40_cxqq(CBRC_BasicInfo basicinfo, CBRC_QueryPerson queryperson, String tasktype) throws Exception {
		/** 查询请求内容 */
		Br40_cxqq br40_cxqq = new Br40_cxqq();
	 ...
	 * @throws Exception
	 */
	public void handleCxqqPz(CBRC_QueryScanResponse query_cgcx, MC21_task_fact mc21_task_fact, MC20_task_msg taskmag) throws Exception {
		CBRC_BasicInfo basicinfo = query_cgcx.getBasicInfo();
		...
		br40_cxqq.setTasktype(tasktype);
		//2.插入请求主体信息、反馈主体信息
		List<CBRC_QueryScanResponse_Info> querymain_cgList = query_cgcx.getScanInfoList();
		for (CBRC_QueryScanResponse_Info querymain_Cg : querymain_cgList) {
			String rwlsh = querymain_Cg.getRwlsh();//任务流水号
			//2.1 插入请求主体信息
			Br40_cxqq_back_pz qqmx = new Br40_cxqq_back_pz();
			BeanUtils.copyProperties(qqmx, querymain_Cg);
			qqmx.setQqdbs(qqdbs);//请求单标识
			qqmx.setTasktype(tasktype);//监管类别
			qqmx.setOrgkey(orgkey);//归属机构
			qqmx.setMbjgdm(mbjgdm);
			qqmx.setSqjgdm(basicinfo.getSqjgdm());
			qqmx.setQrydt(DtUtils.getNowDate());//查询日期
			qqmx.setCxzl(querymain_Cg.getCxzl());
			qqmx.setJylsh(querymain_Cg.getJyls());
			qqmx.setPztxlx(querymain_Cg.getCxzl());
			qqmx.setZtlb(basicinfo.getZtlb());
			qqmx.setXmltype(taskmag.getCode());
			qqmx.setStatus("0");//状态
			qqmx.setMsgcheckresult("1");
			br40_cxqq.setRwlsh(rwlsh);
			br40_cxqqMapper.delBr40_cxqq_pz(br40_cxqq); 
			br40_cxqqMapper.delBr40_cxqq_back_pz_attach(br40_cxqq); 
			br40_cxqqMapper.insertBr40_cxqq_pz(qqmx);
			
			//2.3 插入机构
			String cxzh = querymain_Cg.getCxzh();
			String cxkh = querymain_Cg.getCxkh();
			mc21_task_fact.setFacttablename("" + "#" + mbjgdm);
			mc21_task_fact.setBdhm(qqdbs);
			if (cxzh != null && !cxzh.equals("")) {
				this.insertOrgMc21TaskFact3(mc21_task_fact, rwlsh, "3", cxzh, basicinfo.getQqcslx());
			} 
			if (cxkh != null && !cxkh.equals("")) {
				this.insertOrgMc21TaskFact3(mc21_task_fact, rwlsh, "1", cxkh, basicinfo.getQqcslx());
			} 	
