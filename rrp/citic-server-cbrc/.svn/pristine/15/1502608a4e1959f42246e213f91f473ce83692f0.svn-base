...
public class Cxqq_CLBo extends BaseBo {
	...
	public void handleQueryResponse(MC21_task_fact mc21_task_fact) throws Exception {
		...
		if (msgCheckResult != null && msgCheckResult.equals("1")) { // 判断是否是本行数据	
			...
			try {
				...
				if (StringUtils.equals(this.query_datasource, REMOTE_QUERY)) {
					...
					dataOperate = (IDataOperateCBRC) this.ac.getBean(LOCAL_DATA_OPERATE_NAME_CBRC);
				} else {
					dataOperate = (IDataOperateCBRC) this.ac.getBean(REMOTE_DATA_OPERATE_NAME_CBRC);
				}
				...
				CBRC_QueryRequest_Customer customer = null;
				...
				if (CBRCConstants.CXNR_TRANSACTION.equals(cxnr)) {
					...
					if (customer == null) {
						...
						List<CBRC_QueryRequest_Account> accountList = customer.getAccountList(); // 账户
						if (accountList != null) {
				...
				if (transactionList != null && transactionList.size() > 0) {
					int seq = 1;
					int i = 0;
					List<CBRC_QueryRequest_Transaction> insertList = new ArrayList<CBRC_QueryRequest_Transaction>();
					for (CBRC_QueryRequest_Transaction transaction : transactionList) {
		...
		if (isemployee.equals("0")) { //不走人工	
			 ...
			 * }
			 */
			insertMc21TaskFact3(mc21_task_fact, "CBRC_" + taskType); //直接插入task3一条task2对应一个task3
		}
	...
	public void insertAcct_qzcs_sub_ql(Br40_cxqq_mx br40_cxqq_mx, CBRC_QueryRequest_Customer customer) throws Exception {
		if (StringUtils.equals(this.query_datasource, LOCAL_QUERY)) {
			//4.1.4 插入子账户信息
			//br40_cxqqMapper.insertBr40_cxqq_back_acct_sub(br40_cxqq_mx);
			List<CBRC_QueryRequest_SubAccount> subAccountList = customer.getSubAccountList();
			if (subAccountList != null) {
				for (CBRC_QueryRequest_SubAccount subAccount : subAccountList) {
			...
			List<CBRC_QueryRequest_Measure> measureList = br40_cxqqMapper.getBb11_freeze(br40_cxqq_mx); // 强制措施
			if (measureList != null) {
				int i = 1;
				for (CBRC_QueryRequest_Measure measure : measureList) {
			...
			List<CBRC_QueryRequest_Measure> measureList = customer.getMeasureList(); // 强制措施
			if (measureList != null) {
				int i = 1;
				for (CBRC_QueryRequest_Measure measure : measureList) {
					measure.setQqdbs(qqdbs);
			...
			List<CBRC_QueryRequest_SubAccount> subAccountList = customer.getSubAccountList(); // 子账户
			if (subAccountList != null) {
				for (CBRC_QueryRequest_SubAccount subAccount : subAccountList) {
			...
			List<CBRC_QueryRequest_Priority> priorityList = customer.getPrioritiesList(); // 共有权/优先权
			if (priorityList != null) {
				int n = 1;
				for (CBRC_QueryRequest_Priority priority : priorityList) {
	...
	public void handleQueryResponse_dtcx(MC21_task_fact mc21_task_fact) throws Exception {
		boolean isTaskType5 = ServerEnvironment.TASK_TYPE_GAOJIAN.equals(mc21_task_fact.getTasktype());
		// 1、根据任务信息，获取查询请求信息
		Br40_cxqq br40_cxqq = this.getBr40_cxqq(mc21_task_fact);
		Br40_cxqq_mx br40_cxqq_mx = this.getQueryRequest_mx(br40_cxqq);
		try {
			...
			if (msgCheckResult != null && msgCheckResult.equals("1")) { //判断是否是本行数据		
				...
				if (CBRCConstants.DYNAMIC_CONTINUE.equals(qqcslx)) {
					//查询原动态查询的结束日期
				...
				if (isOk) {
					insertBr40_cxqq_acct_dynamic_main(br40_cxqq_mx);
				} else {
					//修改反馈表状态是失败
	...
	public void insertBr40_cxqq_acct_dynamic_main(Br40_cxqq_mx br40_cxqq_mx) throws Exception {
		// 删除请求单号下的数据
		Br40_cxqq br40_cxqq = new Br40_cxqq();
		String tasktype = br40_cxqq_mx.getTasktype();
		String qqdbs = br40_cxqq_mx.getQqdbs();
		...
		br40_cxqqMapper.delBr40_cxqq_acct_dynamic_main(br40_cxqq);
		...
		if (CBRCConstants.DYNAMIC.equals(qqcslx)) { // 动态查询
			br40_cxqq.setLastpollingtime(mxqssj);
		} else if (CBRCConstants.DYNAMIC_CONTINUE.equals(qqcslx)) { // 继续动态查询
			...
			br40_cxqqMapper.updateBr40_cxqq_acct_dynamic_main(br40_cxqq_mx); // set status_cd = '0'
		...
		br40_cxqqMapper.insertBr40_cxqq_acct_dynamic_main(br40_cxqq);
	}
